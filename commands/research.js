...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			...
			const getWinningSubject = await SubjectsModel.findOne({ where: { subject_id: winner.id, game_id: game_id } })
			const getLosingSubject = await SubjectsModel.findOne({ where: { subject_id: loser.id, game_id } })
			...
			if (loser_sets <= (game.setcount - 1)) {
				...
				if (getWinningSubject && getLosingSubject) {
					...
					if ((interaction.user != interaction.options.getUser('opponent') || config.selfPlay)) {
						...
						if (!duplicateMatchToday || config.duplicates) {
							...
							if ((now.getDay() === 3 && now >= noon && now <= midnight) || config.wednesday) {
								...
								return await interaction.editReply({ content: `Match between ${match.winner_nickname} and ${match.loser_nickname} added with the result of ${game.setcount} - ${loser_sets}.`, components: [row]});
							} else {
					...
					return await interaction.editReply({ content: `You are not registered for this game, please do so with the /register command` })
				} else if (!getLosingSubject) {
					return await interaction.editReply({ content: 'Your opponent is not registered for this game, please have them do so with the /register command'});
				}
			} else {
				return await interaction.editReply({ content: `Loser set count surpasses what is possible in a First to ${game.setcount} set`});
			}
		} catch (error) {
			console.error(error);
