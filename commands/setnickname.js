...
module.exports = {
	...
	async execute(interaction) {
        ...
        const getSubject = await SubjectsModel.findOne({ where: { subject_id: subject_id, game_id: game_id } })
        ...
        if (getSubject) {
            ...
            return await interaction.editReply({ content: `You are not registered for this ${game.name}, please do so with the /register command` })
        }
		return await interaction.editReply({ content: `Nickname updated to ${nickname} for ${game.name}` });
	},
};